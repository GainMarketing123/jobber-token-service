...
3. **Refresh token persisted back to Railway on every refresh — fail closed, persist-first.** Each successful OAuth refresh writes the rotated `JOBBER_REFRESH_TOKEN_<SUB>` back to Railway via API. Without this, the service breaks after the first Jobber-side rotation. The write is `await`ed and validated three ways — HTTP status, GraphQL `errors` array, and an explicit `data.variableUpsert === true` confirmation — before it counts as durable. The in-memory token copy is updated *only after* that durable write confirms (persist-first ordering): a persistence failure leaves both memory and Railway on the previous token, so `GET /token/:subEntity` returns `503` and the next request retries cleanly instead of masking the failure until restart. Exception: if `RAILWAY_API_TOKEN` is unset entirely, persistence is intentionally skipped (local-dev config state, not a failure) and the request still succeeds — production MUST have `RAILWAY_API_TOKEN` set.
